...
- FS-043 cantilever control recovery: the committed recovery state only had old length-only cantilever arrays, so saved FSTR files with local cantilever intent could reopen with wrong/stale controls and full-span cantilever strips. Restored normalized cantilever entries as `{ projection, run, offset, eb }` while keeping numeric backward compatibility. `Run = 0` means full span; `Run > 0` limits the slab strip along the edge; `Off` shifts the local strip start; `EB` toggles outside edge beam generation. Loaded projects must sync the active floor's cantilever set to the UI after hydrate, so a 4x3 model renders top/bottom X1-X4 and left/right Y1-Y3 controls. Generated slabs now carry `width/height` plus `lx/ly`, and structural/tributary slab renderers should fall back to `lx/ly` when needed.
- FS-044 active-only deleted geometry rule: normal Layout/Tributary rendering must not draw red ghost panels for `voidSlabs` or red/dashed markers for inactive/deleted columns. Void/deleted slabs stay in floor data only as deletion intent/history; active area totals, material slab volume, status slab count, tributary hit-testing, and normal slab drawing must filter `!slab.isVoid`. Floor-deleted or inactive columns are treated as absent for generated regular beams, cantilever support-beam availability, endpoint topology, normal 2D rendering, column hit-testing, and per-floor column load accumulation. Long-term recovery should be through undo/restore/debug tools, not through always-visible red ghosts in engineering mode.
- FS-045 WIP/debug checkpoint for Lum review: after FS-044, live browser still shows missing columns in Layout and status bar reports `Init error - check console` even though inline script parsing passes. Screenshot symptoms: only a subset of columns render, column schedule still lists active checked columns, grid/status can degrade to stale `2x2` metadata while spans show the 4x3 project values, and area balance is unstable. Suspect interaction between legacy `col.active`, per-floor `activePerFloor`, `deletedColumns`, loaded-project hydration, and normal active-only display/model filters. This checkpoint is intentionally pushed for external review; do not treat it as stable.
