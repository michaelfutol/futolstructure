...
| MODEL-02 | Persistent column segments, exact columnation and vertical datums | Native Acceptance Pending | Legacy/regular/terminated fixtures and vertical-datum baseline; analytical centroid joints, physical face-terminated 3D beams, ETABS insertion offsets, and STAAD member offsets are now shared; compare current candidate in ETABS/STAAD/Revit |
...
2. Record MODEL-01 regression evidence, then validate a dated Bacacay export against source coordinates, member sizes, analytical centroid joints, and physical member offsets.
